"""
    ...
    `DistanceSE2SubRiemannian`, which can compute the distance map and its
...
class DistanceSE2SubRiemannian():
    """
    ...
    def compute_W(self, C: CostSE2, dxy, dθ, θs_np, n_max=1e5, n_max_initialisation=1e4, n_check=None,
        ...
        References:
            [1]: E. J. Bekkers, R. Duits, A. Mashtakov, and G. R. Sanguinetti.
              ...
              DOI:10.1137/15M1018460.
        """
        W, grad_W = eikonal_solver(C.C, self.source_point, self.ξ, dxy, dθ, θs_np, target_point=self.target_point,
    ...
    def import_W(self, folder):
        ...
        """
        distance_filename = f"{folder}\\SE2_sR_ss_s={[s for s in self.σ_s_list]}_s_o={self.σ_o}_s_s_ext={self.σ_s_ext}_s_o_ext={self.σ_o_ext}_l={self.λ}_p={self.p}_x={self.ξ}_s={self.source_point}.hdf5"
...
class DistanceMultiSourceSE2SubRiemannian():
    """
    Solve the Eikonal PDE on SE(2) equipped with a datadriven left invariant 
    sub-Riemannian metric tensor field defined by `ξ` and `cost_np`, with source
    ...
    def __init__(self, C: CostSE2, ξ, source_points, target_point):
        # Vesselness attributes
        self.σ_s_list = C.σ_s_list
        self.σ_o = C.σ_o
        self.σ_s_ext = C.σ_s_ext
    ...
    def compute_W(self, C: CostSE2, dxy, dθ, θs_np, n_max=1e5, n_max_initialisation=1e4, n_check=None,
        ...
        """
        Solve the Eikonal PDE on SE(2) equipped with a datadriven left invariant 
        sub-Riemannian metric tensor field defined by `ξ` and `cost_np`, with
        source at `source_points`, using the iterative method described by
        Bekkers et al.[1]
        ...
        References:
            [1]: E. J. Bekkers, R. Duits, A. Mashtakov, and G. R. Sanguinetti.
              ...
              DOI:10.1137/15M1018460.
        """
        W, grad_W = eikonal_solver_multi_source(C.C, self.source_points, self.ξ, dxy, dθ, θs_np,
    ...
    def import_W(self, folder):
        ...
        """
        distance_filename = f"{folder}\\SE2_sR_ss_s={[s for s in self.σ_s_list]}_s_o={self.σ_o}_s_s_ext={self.σ_s_ext}_s_o_ext={self.σ_o_ext}_l={self.λ}_p={self.p}_x={self.ξ}.hdf5"
        with h5py.File(distance_filename, "r") as distance_file:
            assert (
                np.all(self.σ_s_list == distance_file.attrs["σ_s_list"]) and
                self.σ_o == distance_file.attrs["σ_o"] and
