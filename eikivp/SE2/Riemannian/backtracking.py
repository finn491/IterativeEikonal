...
def geodesic_back_tracking_sub_Riemannian(grad_W_np, source_point, target_point, cost_np, xs_np, ys_np, θs_np, ξ,
    ...
    # Set hyperparameters
    G = np.identity(3)# ti.Matrix(G_np, ti.f32)
    if dt is None:
        # It would make sense to also include G somehow, but I am not sure how.
        dt = cost_np.min()
...
@ti.kernel
def geodesic_back_tracking_sub_Riemannian_backend(
    grad_W: ti.template(),
    source_point: ti.types.vector(3, ti.f32),
    target_point: ti.types.vector(3, ti.f32),
    θs: ti.template(),
...
# Plus controller backtracking

def geodesic_back_tracking_plus(grad_W_np, source_point, target_point, cost_np, xs_np, ys_np, θs_np, G_np, dt=None, β=0., n_max=10000):
    """
    Find the geodesic connecting `target_point` to `source_point`, using 
    gradient descent back tracking, as described in Bekkers et al. "A PDE 
    approach to Data-Driven Sub-Riemannian Geodesics in SE(2)" (2015).
...
@ti.kernel
def geodesic_back_tracking_plus_backend(
    grad_W: ti.template(),
    source_point: ti.types.vector(3, ti.f32),
    target_point: ti.types.vector(3, ti.f32),
    θs: ti.template(),
